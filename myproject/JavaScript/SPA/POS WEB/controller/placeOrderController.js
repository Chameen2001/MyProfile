...
var add_to_cart = function () {
  ...
  let result = isExist(
    $("#item-selectText").text().split("(")[1].split(")")[0]
  );
...
var generate_order_id = function () {
  if (orders.length != 0) {
    let lastID = parseInt(
      orders[orders.length - 1].get_order_id().split("O")[1]
    );
    if (lastID < 9) {
      return "O00" + (lastID + 1);
    } else if (lastID < 99) {
...
$("#pay_now").on("click", function () {
  ...
  order1.set_total_price($("#lbl_total").text());
...
var clearAll = function () {
  ...
  discount = 0.0;
  total = 0.0;
  $("#selectText").text("Select Customer");
  $("#item-selectText").text("Select Customer");
  $("#txt_ph_cust_name").val("");
  $("#txt_ph_cust_address").val("");
  $("#txt_ph_item_name").val("");
  $("#txt_ph_item_stock").val("");
  $("#txt_ph_item_unit_price").val("");
  $("#txt_ph_item_discount").val("");
  $("#txt_ph_item_qty").val("");

  $("#tbl_cart").empty();

  $("#lbl_sub_total").text("Rs.0.00");
  $("#lbl_discount").text("Rs.0.00");
